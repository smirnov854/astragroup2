function reInit() {
    ...
    $(document).click(function (e) {
        var select = $(".main__cruise__drop.select");
        var div = $(".main__cruise__drop__wrap");
        var podbor = $('.main__cruise__button.main__cruise-podbor');
        var reset = $('.main__cruise__button.main__cruise-filtr');

        // console.log(select.length);
        if (select.length && !div.is(e.target) && div.has(e.target).length === 0 && !podbor.is(e.target) && !reset.is(e.target)) {
            select.children('.main__cruise__drop__wrap').fadeOut(300);
            select.children('.main__cruise__drop__title').val('');
    ...
    $('.main__cruise__drop').click(function (e) {
        var select = $(".main__cruise__drop.select");
        if (!select.length && $(this).children('.main__cruise__drop__wrap').is(':hidden')) {
            $(e.target).parent('.main__cruise__drop').children('.main__cruise__drop__wrap').fadeIn(300);
            $(e.target).parent('.main__cruise__drop').addClass('select');
            return false;
        }
    });
    ...
    /*
        $('.main__cruise__drop').click(function (e) {
            var div = $(".main__cruise__drop__wrap");
            $('.main__cruise__drop.select').children('.main__cruise__drop__wrap').fadeOut(300);
            $('.main__cruise__drop.select').removeClass('select');
    ...
    /*
        $('.main__cruise__date__end ').click(function () {
            $(this).parent().fadeOut(300);
            $(this).parent().parent().removeClass('select');
        });
    */
    ...
    $('.sidebar__close').click(function () {
        ...
        ;
        $('.overlay').css('display', 'none');
        $('.main__cruise__button.main__cruise-podbor').click();
    });
    $('.overlay').click(function () {
        $('.mobile__menu').animate({ 'left': '-350px' }, 800);
        $(this).css('display', 'none');
    ...
    var minprice = $("#slider").data("min"),
        maxprice = $("#slider").data("max"),
        price1 = $("#slider").data("val1"),
        price2 = $("#slider").data("val2");
    ...
    $("#slider_1").slider({
        range: true,
        min: $('#slider_1').data('min'),
        max: $('#slider_1').data('max'),
        step: 1,
        values: [$('#slider_1').data('val1'), $('#slider_1').data('val2')],
        change: function (event, ui) {
    ...
    $('.book__left__item__table__tr').click(function () {
        if ($(this).children('.book__left__item__table__tr__open').is(':hidden')) {
            $(this).children('.book__left__item__table__tr__open').fadeIn(200).css('display', 'flex');
            if (ww < 992) {
                ...
                $(this).children('.book__left__item__table__tr__open').insertAfter('.book__left__bottom__right');
                $(this).addClass('active');
            }
        } else {
            $(this).children('.book__left__item__table__tr__open').fadeOut(200).css('display', 'none');
 ...
 arrows:true
    ...
    if (ww < 992) {
        ...
        $('.book__left__bottom__item--mobile__sel').click(function () {
            if ($(this).parent().siblings('.book__left__bottom__select').is(':hidden')) {
                $(this).parent().siblings('.book__left__bottom__select').fadeIn(200);
            } else {
                $(this).parent().siblings('.book__left__bottom__select').fadeOut(200);
            }
        });

        $('.book__left__bottom__item').each(function () {
            $(this).on('click', function () {
                $('.book__left__bottom__item--mobile').replaceWith(this);
            });
        });
    ...
    $('input[type=search]').keyup(function () {
        var word = $(this).val();
        console.log(word);
        var list = $(this).parent().find('.main__cruise__drop__region');
        list.each(function (index, value) {
            if ($(list[index]).find('span.label').text().toLowerCase().indexOf(word.toLowerCase()) < 0) {
                $(list[index]).hide();
            }
            else {
                $(list[index]).show();
    ...
    // $(function () {
    //     $('.main__cruise input[type=date]').on('change', function () {
    //         if ($(this).val().length == 10) {
    //             var currentPicker = $(this).attr('name');
    //             var dateFrom = Date.parse($('.main__cruise input[name=date_from]').val());
    //             var dateTo = Date.parse($('.main__cruise input[name=date_to]').val());
    //             // console.log(dateFrom);
    //             // console.log(dateTo);
    //             if (dateFrom > dateTo) {
    //                 var newTime = 0, newDate = '';
    //                 if ($(this).attr('name') === 'date_from') {
    //                     newTime = dateFrom + 35 * 24 * 3600000;
    //                     newDate = ("" + (new Date(newTime)).toISOString()).replace(/^([^T]+)T(.+)$/, '$1');
    //                     $('.main__cruise input[name=date_to]').val(newDate);
    //                 }
    //                 else {
    //                     newTime = dateTo - 24 * 3600000;
    //                     newDate = ("" + (new Date(newTime)).toISOString()).replace(/^([^T]+)T(.+)$/, '$1');
    //                     $('.main__cruise input[name=date_from]').val(newDate);
    //                 }
    //             }
    //             $('.main__cruise__button.main__cruise-podbor').click();
    //         }
...
function checkSearchForm(event) {
    let error = false;
    let elmts = ['.main__cruise__date__start', '.main__cruise__date__end'];
...
function updateSearchFormFilters() {
    ...
    $('.field-regions div span.region-type').each(function (index) {
        // if (item.data('regionid') === field.value);
        if (values.indexOf(+$(this).data('regionid')) === -1 && values.length !== 0) {
            $(this).closest('.main__cruise__drop__region').hide();
        } else {
            $(this).closest('.main__cruise__drop__region').show();
        }
    })
}


$(document).on('submit', '#w1, #w0', checkSearchForm);
$(document).on('change', '#w0, #w1', updateSearchFormFilters);

$(document).on('change', '.main__cruise__date__start.form-error, .main__cruise__date__end.form-error', checkSearchForm);
$(document).on('change', '.main__cruise__date__start', () => {
    if ($('.main__cruise__date__end').val() == "") {
        $('.main__cruise__date__end').attr('min', $('.main__cruise__date__start').val() == "" ? $('.main__cruise__date__start').attr('min') : $('.main__cruise__date__start').val());
    } else {
        var dateStart = Date.parse($('.main__cruise__date__start').val());
        var dateEnd = Date.parse($('.main__cruise__date__end').val());

        if (dateStart > dateEnd) {
            $('.main__cruise__date__end').val("");
            $('.main__cruise__date__end').attr('min', $('.main__cruise__date__start').val());
        }
    }
});

$(document).on('click', '.form__result__empty', () => {
    $('.form__result__button--mobile').click();
});
